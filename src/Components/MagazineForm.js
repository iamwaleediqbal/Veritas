...
class MagazineForm extends React.Component{
    ...
    handleUpload = (e) =>{
        ...
        const uploadTask = fireBaseApp.storage().ref(`magazines/covers/${this.images[0].name}`).put(this.images[0]);
        fireBaseApp.storage().ref(`magazines/ebooks/${this.images[1].name}`).put(this.images[1]);
        const docId = fireBaseApp
        .firestore()
        .collection('magazines').doc();
    ...
    addMessage = (e) =>{
        ...
        img.ref()
        db.collection("magazines").doc().set({
    ...
    render(){
        ...
        return(
            <div> 
               ...
               <h3 align = 'center' className = "heading" >Add A Magazine</h3>
                <div className="container" id="container">
                    <div className=" form-container log-in-container">
                        <form className = "bookForm"onSubmit={this.addMessage.bind(this)}><br/><br/>
                            ...
                            <br/>
                            <input className = "inputcat" type="date" value = {this.date} onChange={(e) => { this.setState({ date: e.target.value }) }} placeholder="Date" />
                            <br/>
                            <input className = "inputcat"type="text" value = {this.description} onChange={(e) => { this.setState({ description: e.target.value }) }} placeholder="Description" />
                        </form>
                    </div>
